...
set(BUILD_ANTILLA_DEFAULT_LOCATION ON)
...
#Build Antilla
add_subdirectory(3rdparty/Antilla)
...
set(QML_IMPORT_PATH ${CMAKE_SOURCE_DIR}/3rdparty/Antilla/imports CACHE STRING "" FORCE)
target_compile_definitions(${PROJECT_NAME} PRIVATE
    ANTILLA_IMPORT_PATH="${CMAKE_SOURCE_DIR}/3rdparty/Antilla/imports"
)
)";
...
static auto g_cmake_only_link_antilla = R"(
target_link_libraries(${PROJECT_NAME} PRIVATE
    Qt6::Quick
    AntillaBasic
...
static auto g_cmake_link_antilla = R"(
...
int main(int argc, char *argv[])
{
    ...
    QObject::connect(
        ...
        []() { QCoreApplication::exit(-1); },
        Qt::QueuedConnection);
    engine.addImportPath(ANTILLA_IMPORT_PATH);
    engine.loadFromModule("%1", "Main");
