...
titleNames | array | ['Source', 'Target'] | 列表标题名称，顺序从左至右
titleNamesVisible | bool | false | 是否显示列表标题名称
operations | array | ['>', '<'] | 操作文案集合，顺序从左至右
searchVisible | bool | false | 是否显示搜索框
filterOption | function(value, record) | - | 输入项将使用该函数进行筛选(searchVisible需为true)
itemsSuffix | string | '项' | 条目项后缀
emptyDescription | string | '暂无数据' | 空数据的描述
searchPlaceholder | string | '搜索' | 搜索框占位符
pagination | bool丨object | false | 是否使用分页样式
unidirection | bool | false | 是否单向穿梭
titleFont | font | - | 标题文本
...
sourceTableView | [AntTableView](internal://AntTableView) | - | 访问内部左侧表格视图
targetTableView | [AntTableView](internal://AntTableView) | - | 访问内部右侧表格视图
\n<br/>
\n### {dataSource}支持的属性：\n
属性名 | 类型 | 可选/必选 | 描述
------ | --- | :---: | ---
...
\n<br/>
\n### 支持的函数：\n
- \`clearChecked(direction: string = 'left')\` 清除 \`direction\` 指定方向的所有选中键。\n
...
- \`change(nextTargetKeys: var, direction: string, moveKeys: var)\` 选项在两栏之间转移时发出\n
        ...
        Description {
            ...
            desc: qsTr(`
- 需要在多个可选项中进行多选时。\n
- 比起 [AntSelect](internal://AntSelect) 和 [AntTreeSelect](internal://AntTreeSelect)，穿梭框占据更大的空间，可以展示可选项的更多信息。\n
...
最基本的用法，展示了 \`dataSource\`、\`targetKeys\`、\`defaultTargetCheckedKeys\` 的用法。\n
        ...
        CodeBox {
            ...
            descTitle: qsTr('带搜索框')
            desc: qsTr(`
通过 \`searchVisible\` 设置为 \`true\` 显示搜索框。\n
...
- pageSize 每页数量。\n
- pageMaxButton 最大页按钮数。\n
- quickJumperVisible 是否显示快速跳转。\n
详细说明见 [AntPagination](internal://AntPagination)。\n
                       `)
            code: `
                import QtQuick
                import Antilla.Basic
