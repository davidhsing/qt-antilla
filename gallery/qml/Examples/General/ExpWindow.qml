...
captionBar | [AntCaptionBar](internal://AntCaptionBar) | - | 窗口标题栏
windowAgent | AntWindowAgent | - | 窗口代理
followThemeSwitch | bool | true | 是否跟随系统明/暗模式自动切换
initialized | bool | false | 指示窗口是否已经初始化完毕
specialEffect | enum | AntWindow.EffectNone | 特殊效果(来自 AntWindow)
\n<br/>
\n### 支持的函数：\n
...
- \`setSpecialEffect(specialEffect: int): bool\` 设置窗口的特殊效果\n
        ...
        Description {
            title: qsTr('何时使用')
            desc: qsTr(`
当用户需要自定义窗口形态时作为基础无边框窗口使用，默认提供一个 [AntCaptionBar](internal://AntCaptionBar)。
                       `)
        }
        ...
        CodeBox {
            ...
            desc: qsTr(`
使用方法等同于 \`Window\`\n
**注意** 不要嵌套使用 AntWindow (源于Qt的某些BUG)：\n
\`\`\`qml
AntWindow {
    AntWindow { }
}
