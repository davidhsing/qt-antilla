...
windowAgent | AntWindowAgent | null | 无边框窗口代理(通常不使用)
...
当无边框窗口需要一个通用的标题栏时使用。
        ...
        CodeBox {
            width: parent.width
            desc: qsTr(`
\`AntWindow\` 自带一个 \`AntCaptionBar\`，通常不需要单独使用。
                       `)
        }
    }
}
