...
Item {
    ...
    property int size: AntButtonBlock.SizeAuto
    ...
    property Component buttonDelegate: AntIconButton {
        ...
        implicitWidth: control.size == AntButtonBlock.SizeAuto ? (implicitContentWidth + leftPadding + rightPadding) :
                                                                 control.buttonWidth
        implicitHeight: control.size == AntButtonBlock.SizeAuto ? (implicitContentHeight + topPadding + bottomPadding) :
                                                                  control.buttonHeight
        z: (hovered || checked) ? 1 : 0
        enabled: control.enabled && (modelData.enabled === undefined ? true : modelData.enabled)
        font: control.font
        type: modelData.type ?? AntButton.TypeDefault
